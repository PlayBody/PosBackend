...
class UpdateController extends CI_Controller
{
	...
	ob_start();
        ...
        if (!$this->_login_check($role)) {
            if ($role == ROLE_STAFF) redirect('/login');
        }
    }

    function logout($role = ROLE_GUEST)
    {
        switch ($role) {
            case ROLE_STAFF:
...
//    }
    ...
    function load_view_with_menu($viewName = "", $prefix = "")
    {
        ...
        $this->load->view($prefix . 'includes/withmenu_kiaalap', $this->header);
    }
    function load_view_empty($viewName = "", $prefix = "")
    {
        $this->header['contents'] = $this->load->view($viewName, $this->data, true);
        $this->load->view($prefix . 'includes/empty', $this->header);
    }
